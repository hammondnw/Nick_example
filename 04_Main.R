...
if(file.exists("01_download_phenocam.R")){
  source("01_download_phenocam.R")
}
if(file.exists("02_plot_phenocam.R")){
  source("02_plot_phenocam.R")
}
if(file.exists("03_logistic.R")){
  source("03_logistic.R")
}          
...
fit <- fit_logistic(dat,par)
pred <- tibble(date = dat$date,
               pred = pred_logistic(fit$par, dat$doy))
